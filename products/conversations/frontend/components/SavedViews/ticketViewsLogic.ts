import { MakeLogicType, actions, connect, kea, key, listeners, path, props, reducers, selectors } from 'kea'
import { loaders } from 'kea-loaders'

import { lemonToast } from '@posthog/lemon-ui'

import { createFuseSearch } from 'lib/utils/fuseSearch'
import { teamLogic } from 'scenes/teamLogic'

import {
    conversationsViewsCreate,
    conversationsViewsDestroy,
    conversationsViewsList,
    conversationsViewsPartialUpdate,
} from '../../generated/api'
import type { PatchedTicketViewApi, TicketViewApiFilters } from '../../generated/api.schemas'
import { supportTicketsSceneLogic } from '../../scenes/tickets/supportTicketsSceneLogic'
import type { SavedTicketView, TicketViewFilters } from '../../types'

export interface TicketViewsLogicProps {
    id: string
}

export interface TicketViewChanges {
    name?: string
    filters?: TicketViewFilters
}

const searchViews = createFuseSearch<SavedTicketView>(['name'])

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface ticketViewsLogicValues {
    activeView: SavedTicketView | null // supportTicketsSceneLogic
    currentFilters: TicketViewFilters // supportTicketsSceneLogic
    currentTeamId: number | null // teamLogic
    filteredViews: SavedTicketView[]
    isModalOpen: boolean
    isSaveModalOpen: boolean
    searchTerm: string
    viewName: string
    views: SavedTicketView[]
    viewsLoading: boolean
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface ticketViewsLogicActions {
    applyViewFilters: (filters: TicketViewFilters) => {
        filters: TicketViewFilters
    } // supportTicketsSceneLogic
    setActiveView: (view: SavedTicketView | null) => {
        view: SavedTicketView | null
    } // supportTicketsSceneLogic
    closeModal: () => {
        value: true
    }
    closeSaveModal: () => {
        value: true
    }
    createView: ({ name, filters }: { filters: TicketViewFilters; name: string }) => {
        name: string
        filters: TicketViewFilters
    }
    createViewFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    createViewSuccess: (
        views: SavedTicketView[],
        payload?: {
            name: string
            filters: TicketViewFilters
        }
    ) => {
        views: SavedTicketView[]
        payload?: {
            name: string
            filters: TicketViewFilters
        }
    }
    deleteView: (shortId: string) => {
        shortId: string
    }
    loadView: (view: SavedTicketView) => {
        view: SavedTicketView
    }
    loadViews: () => any
    loadViewsFailure: (
        error: string,
        errorObject?: any
    ) => {
        error: string
        errorObject?: any
    }
    loadViewsSuccess: (
        views: SavedTicketView[],
        payload?: any
    ) => {
        views: SavedTicketView[]
        payload?: any
    }
    openModal: () => {
        value: true
    }
    openSaveModal: () => {
        value: true
    }
    saveView: () => {
        value: true
    }
    setSearchTerm: (searchTerm: string) => {
        searchTerm: string
    }
    setViewName: (viewName: string) => {
        viewName: string
    }
    updateView: (
        shortId: string,
        changes: TicketViewChanges
    ) => {
        changes: TicketViewChanges
        shortId: string
    }
    viewUpdated: (view: SavedTicketView) => {
        view: SavedTicketView
    }
}

// Generated by kea-typegen. Update if you're an agent, ignore if you're human.
export interface ticketViewsLogicMeta {
    key: string
    __keaTypeGenInternalSelectorTypes: {
        filteredViews: (views: SavedTicketView[], searchTerm: string) => SavedTicketView[]
    }
}

export type ticketViewsLogicType = MakeLogicType<
    ticketViewsLogicValues,
    ticketViewsLogicActions,
    TicketViewsLogicProps,
    ticketViewsLogicMeta
>

export const ticketViewsLogic = kea<ticketViewsLogicType>([
    props({} as TicketViewsLogicProps),
    key((props) => props.id),
    path((key) => ['products', 'conversations', 'frontend', 'components', 'SavedViews', 'ticketViewsLogic', key]),

    connect(() => ({
        values: [teamLogic, ['currentTeamId'], supportTicketsSceneLogic, ['currentFilters', 'activeView']],
        actions: [supportTicketsSceneLogic, ['applyViewFilters', 'setActiveView']],
    })),

    actions({
        deleteView: (shortId: string) => ({ shortId }),
        updateView: (shortId: string, changes: TicketViewChanges) => ({ shortId, changes }),
        viewUpdated: (view: SavedTicketView) => ({ view }),
        loadView: (view: SavedTicketView) => ({ view }),
        openModal: true,
        closeModal: true,
        openSaveModal: true,
        closeSaveModal: true,
        setViewName: (viewName: string) => ({ viewName }),
        saveView: true,
        setSearchTerm: (searchTerm: string) => ({ searchTerm }),
    }),

    loaders(({ values }) => ({
        views: [
            [] as SavedTicketView[],
            {
                loadViews: async () => {
                    const response = await conversationsViewsList(String(values.currentTeamId))
                    return response.results as unknown as SavedTicketView[]
                },
                createView: async ({ name, filters }: { name: string; filters: TicketViewFilters }) => {
                    const created = (await conversationsViewsCreate(String(values.currentTeamId), {
                        name,
                        filters: filters as TicketViewApiFilters,
                    })) as unknown as SavedTicketView
                    lemonToast.success('View saved')
                    return [created, ...values.views]
                },
            },
        ],
    })),

    reducers({
        views: {
            deleteView: (state, { shortId }) => state.filter((v) => v.short_id !== shortId),
            viewUpdated: (state, { view }) => state.map((v) => (v.short_id === view.short_id ? view : v)),
        },
        isModalOpen: [
            false,
            {
                openModal: () => true,
                closeModal: () => false,
                loadView: () => false,
            },
        ],
        isSaveModalOpen: [
            false,
            {
                openSaveModal: () => true,
                closeSaveModal: () => false,
                createViewSuccess: () => false,
            },
        ],
        viewName: [
            '',
            {
                setViewName: (_, { viewName }) => viewName,
                closeSaveModal: () => '',
                createViewSuccess: () => '',
            },
        ],
        searchTerm: [
            '',
            {
                setSearchTerm: (_, { searchTerm }) => searchTerm,
                openModal: () => '',
                closeModal: () => '',
            },
        ],
    }),

    selectors({
        filteredViews: [
            (s) => [s.views, s.searchTerm],
            (views: SavedTicketView[], searchTerm: string): SavedTicketView[] => searchViews(views, searchTerm),
        ],
    }),

    listeners(({ actions, values }) => ({
        openModal: () => {
            actions.loadViews()
        },
        saveView: () => {
            const name = values.viewName.trim()
            if (!name) {
                return
            }
            actions.createView({ name, filters: { ...values.currentFilters } })
        },
        deleteView: async ({ shortId }) => {
            try {
                await conversationsViewsDestroy(String(values.currentTeamId), shortId)
                lemonToast.success('View deleted')
            } catch {
                lemonToast.error('Failed to delete view')
                actions.loadViews()
            }
        },
        updateView: async ({ shortId, changes }) => {
            try {
                const updated = (await conversationsViewsPartialUpdate(
                    String(values.currentTeamId),
                    shortId,
                    changes as PatchedTicketViewApi
                )) as unknown as SavedTicketView
                actions.viewUpdated(updated)
                // Keep the header indicator and URL state in sync when the loaded view changes
                if (values.activeView?.short_id === shortId) {
                    actions.setActiveView(updated)
                }
                lemonToast.success('View updated')
            } catch {
                lemonToast.error('Failed to update view')
                actions.loadViews()
            }
        },
        loadView: ({ view }) => {
            actions.applyViewFilters(view.filters || {})
            actions.setActiveView(view)
        },
        createViewFailure: () => {
            lemonToast.error('Failed to save view')
        },
        loadViewsFailure: () => {
            lemonToast.error('Failed to load saved views')
        },
    })),
])
