...
// Query-param keys owned by the tickets filters. On every filter change the URL
// is rewritten from state, so these are cleared first and unrelated params (the
// saved-view `view` shortcut, anything else) are preserved. Date range is
// deliberately excluded — it's a personal persisted preference, not part of the
// shareable filter set.
...
function urlFiltersMatchState(searchParams: Record<string, any>, currentFilters: TicketViewFilters): boolean {
    return objectsEqual(filtersToUrlParams(urlParamsToFilters(searchParams)), filtersToUrlParams(currentFilters))
}
...
export const supportTicketsSceneLogic = kea<supportTicketsSceneLogicType>([
    ...
    urlToAction(({ actions, values, props }) => ({
        '/support/tickets': (_, searchParams) => {
            ...
            // Only react when the URL actually carries filters, so navigating to a bare
            // /support/tickets keeps the persisted selection instead of wiping it.
            if (hasFilterParams(searchParams) && !urlFiltersMatchState(searchParams, values.currentFilters)) {
                actions.applyViewFilters(urlParamsToFilters(searchParams))
            }
        },
    })),
