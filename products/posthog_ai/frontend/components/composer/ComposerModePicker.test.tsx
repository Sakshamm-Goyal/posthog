...
describe('ComposerModePicker', () => {
    ...
    it('shows the three product modes with one description at a time and emits their server values', () => {
        ...
        expect(
            screen.getByText('Edits and commands run freely. Asks before changing live PostHog data.')
        ).toBeInTheDocument()
        expect(
        ...
        expect(
            screen.queryByText('Edits and commands run freely. Asks before changing live PostHog data.')
        ).not.toBeInTheDocument()
