...
class BatchConsumerAdapter(Protocol):
    ...
    async def update_status(
        ...
        *,
        ...
        where supported; a 0 return means the state moved and nothing was written."""
        ...
            ...
            for batch in stale:
                ...
                try:
                    ...
                    if batch.latest_attempt >= self._config.max_attempts:
                        ...
                        updated = await self._adapter.update_status(
                            conn,
                            batch_id=batch.id,
                            job_state=self._adapter.waiting_retry_state,
                            attempt=batch.latest_attempt,
                        ...
                        if updated == 0:
                            # The state moved after the stale scan (e.g. the owner finished): leave it.
                            logger.warning(
                                self._event("recovery_requeue_skipped_state_moved"), attempt=batch.latest_attempt
                            )
                        else:
